   ...
   public function update(Request $request, Employer $employer)
{
    ...
    if ($request->hasFile('logo')) {
        ...
        // Lưu logo mới và gán vào field avatar
        $logoPath = $request->file('logo')->store('avatars', 'public');
        $validated['avatar'] = $logoPath; // Lưu vào field avatar, không phải logo
    }

    // Thêm slug
    $validated['slug'] = Str::slug($validated['company_name']);

    // Loại bỏ field 'logo' khỏi validated array vì ta đã xử lý riêng
